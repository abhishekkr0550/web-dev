...
eesy.define(['jquery-private', 'sessionInfo'], function ($, sessionInfo) {
    document.addEventListener("registerLtiLaunch", function (d) {
        var ltiLaunchWithCourseId = {
            data: d.detail.data,
            launch_type: d.detail.launchType,
            courseId: d.detail.courseId !== undefined ? d.detail.courseId : -1
        };
    ...
    function handleLtiLaunch(ltiLaunchWithCourseId) {
        ...
        $.ajax({
            url: sessionInfo.dashboardUrl() + "/rest/public/lti/launch" +
                ("?launchtype=" + ltiLaunchWithCourseId.launch_type + "&data=" + ltiLaunchWithCourseId.data + "&sessionkey=" + sessionInfo.sessionKey() + "&courseid=" + ltiLaunchWithCourseId.courseId + "&_=" + new Date().getTime()),
            type: 'PUT',
    ...
    return {
        handleUnhandledLtiLaunches: handleUnhandledLtiLaunches
    };
});
