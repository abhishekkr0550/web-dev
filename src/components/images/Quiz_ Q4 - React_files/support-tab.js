...
eesy.define([
    ...
    'json!language', 'helpitem-handling', 'context-tree-matcher', 'context-probe', 'sessionInfo'
], function ($, engineState, utils, mouse, Mustache, language, helpitemHandling, contextTreeMatcher, ctxProbe, sessionInfo) {
    var onClickSupportTab;
    var supportTabAlign = 'right';
    // install global launch listener
    $(document).on('click', '#eesy-tab-inner .eesy-tab2-btn-content, #eesy-tab-inner.eesy-tab-v1', function () {
        if ($("#buildModeBar").length == 0) {
            onClickSupportTab();
        }
    });
    $(document).on('keypress', '#eesy-tab-inner .eesy-tab2-btn-content, #eesy-tab-inner.eesy-tab-v1', function (e) {
    ...
    function show(_onClickSupportTab) {
        ...
        if (var_moveable_tab === true) {
            var $supportButtonElement = $('#eesy-tab-inner');
            if (supportTabPosition !== null) {
                $supportButtonElement.css({ 'top': supportTabPosition + 'px' });
                if (!utils.isInViewport($supportButtonElement)) {
                    $supportButtonElement.css({ 'top': '' });
                }
            }
            if (var_tab_version === 3)
                $supportButtonElement.css({ 'right': '-25px' });
            else if (var_uefModeOriginal)
                $supportButtonElement.css({ 'right': '-15px' });
