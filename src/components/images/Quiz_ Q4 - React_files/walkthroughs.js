...
eesy.define([
    ...
    var STORAGE_KEY = "eesysoft_walkthroughs_" + getUserId();
    ...
    $(document).on('click', EXIT_CONFIRM_SELECTOR + " button.cancel-btn", onWalkExitCancelBtnClick);
    ...
    function markStepAsCompleted(walkId, stepId) {
        $.ajax({
            type: 'POST',
            url: getProgressUrl("/" + walkId + "/completed/" + stepId),
        });
    }
    ...
    function markWalkAsCompleted(walkId) {
        $.ajax({
            type: 'PUT',
            url: getProgressUrl("/" + walkId + "/completed"),
        });
    }
    /**
    ...
    function onVoteBtnClick(ev) {
        ...
        if (settings.SUPPORTCENTER.VOTINGOPTIONS.ENABLED) {
            ...
            var optionsRoute = sessionInfo.dashboardUrl() + "/rest/helpitems/options/voting";
            var optionsUrl = optionsRoute + "?type=" + voteType + "&sessionkey=" + sessionInfo.sessionKey();
    ...
    function onTransitionsBtnClick(ev) {
        ...
        switch (type) {
            ...
            default:
                throw new Error("Not supported transition type \"" + type + "\"!");
        }
    }
